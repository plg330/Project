...
public class Easy_Tricep extends AppCompatActivity {
    ...
    private boolean isAlternating = false; // To track if alternating is in progress
    private Handler handler = new Handler();

    // Image sets
    private int[][] allImages = {
            {R.drawable.cable_rope_overhead_extension_rest, R.drawable.cable_rope_overhead_extension_up}, // images1
            {R.drawable.reverse_grip_tricep_pushdown_rest, R.drawable.reverse_grip_tricep_pushdown},     // images2
            {R.drawable.tricep_push_down, R.drawable.tricep_push_down_up} // images3
    };

    // Text sets (reduced to one text per set)
    private String[] allTexts = {
            "CABLE ROPE OVERHEAD TRICEP EXTENSION 12X4",    // text for images1
            "REVERSE GRIP TRICEP PUSHDOWN 12X4",      // text for images2
            "TRICEP PUSHDOWN" // text for images3
    };
    ...
    private void startAlternatingImages() {
        ...
        final Runnable runnable = new Runnable() {
            @Override
            public void run() {
                if (iterationCount[0] < 10) { // 10 iterations for 5 pairs (5 times alternating between 0 and 1)
